...
module.exports = register => {
	register("snipe", (message, args, client) => {
		const filtered = client.snipes
			.filter(snipe => snipe.time.getTime() > new Date().getTime() - 60000)
			.slice(0, 5)
		client.snipes = []
		if (filtered.length > 0) {
			message.channel.send(
				`${filtered.length} snipe${
					filtered.length === 1 ? "" : "s"
				}. Sniped by ${message.author.toString()}`
			)
			filtered.forEach(snipe =>
				message.channel.send({ embed: formatters[snipe.type](snipe) })
			)
		} else message.channel.send("Nobody to snipe.")
	})
}
